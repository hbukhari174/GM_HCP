...
Commons.index = (1:height(Commons)).';
Z = Commons;
Zcheck = Commons;
...
Z.OffDiagCounts = [];
Commons = repmat(Commons,997,1);
Z = repmat(Z,997,1);
Z.Properties.VariableNames([1 2 3 4 5]) = {'Subject ID 2' 'cfc' 'sexii' 'ageii' 'indx'};
...
Swapsbetweenpairs{i} = mat2cell((triu(dist(fp,sp))),1,1);
end
...
%2=cfc of person1, 7 = cfc of person 2, 12 is swaps between them
...
% accumarray(jinta,1)
% Uniquescorepairstable(:,4) = ans
% 
% Uniquescorepairstable(Uniquescorepairstable(:, 3) == 0, :) = [];
% interpolation between scores for surf figure
...
xlabel('CFC score')
ylabel('CFC score')
title("Average Swaps by Pairs of CFC score - binned by  5")
colormap(parula)
% % % xlim([1.5 16.5])
% % % ylim([1.5 16.5])
